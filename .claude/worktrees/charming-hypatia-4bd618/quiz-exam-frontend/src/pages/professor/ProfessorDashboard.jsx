  ...
  useEffect(() => {
    examsApi.list()
      .then(data => {
        const list = data || []
        setExams(list)
        // Tally results across all professor's exams (professor-accessible endpoint)
        Promise.all(list.map(ex => resultsApi.byExam(ex.id).catch(() => [])))
          .then(all => setResultCount(all.flat().length))
      })
      .catch(() => {})
