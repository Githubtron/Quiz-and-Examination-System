...
public class ExamController {
    ...
    @PostMapping("/{id}/papers/batch-generate")
    @PreAuthorize("hasAnyRole('PROFESSOR','ADMIN')")
    public ResponseEntity<?> batchGeneratePapers(@PathVariable long id,
                                                  @RequestBody BatchGenerateRequest req) {
        if (!examRepository.existsById(id)) return ResponseEntity.notFound().build();
        if (req.studentIds() == null || req.studentIds().isEmpty()) {
            return ResponseEntity.badRequest().body(Map.of("error", "studentIds required"));
        }
        try {
            return ResponseEntity.ok(examTemplateService.generateAllPapers(id, req.studentIds()));
        } catch (IllegalStateException e) {
            return ResponseEntity.status(503).body(Map.of("error", e.getMessage()));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
    ...
    @PreAuthorize("hasAnyRole('PROFESSOR','ADMIN')")
    public ResponseEntity<?> listPapers(@PathVariable long id) {
        if (!examRepository.existsById(id)) return ResponseEntity.notFound().build();
        return ResponseEntity.ok(examTemplateService.listPapers(id));
    }

    @GetMapping("/{id}/papers/{studentId}/export")
    @PreAuthorize("hasAnyRole('PROFESSOR','ADMIN')")
    public ResponseEntity<?> exportPaper(@PathVariable long id,
                                          @PathVariable long studentId,
                                          @RequestParam(defaultValue = "") String label) {
        if (!examRepository.existsById(id)) return ResponseEntity.notFound().build();
        try {
            return ResponseEntity.ok(
                examTemplateService.exportPaper(id, studentId, label.isBlank() ? "S?" : label)
            );
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    ...
    public record BatchGenerateRequest(List<Long> studentIds) {}
}
